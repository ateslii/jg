...
export const validationSchemaBooking = () => {
  return Yup.object().shape({
    name: Yup.string()
      ...
      .matches(
        nameRegExp,
        'Пожалуйста, введите правильное имя без пробелов в начале'
      )
      .required('Имя обязательно'),
    email: Yup.string()
      .matches(emailRegExp, 'Пожалуйста, введите действительный адрес электронной почты')
      .required('Электронная почта обязательна'),
    phone: Yup.string()
      .matches(phoneRegExp, 'Пожалуйста, введите действительный номер телефона')
      .required('Номер телефона обязателен'),
    service: Yup.string().required('Требуется обслуживание'),
    slot: Yup.string().required('Дата и время обязательны'),
  });
};
...
export const validationSchemaClient = () => {
  return Yup.object().shape({
    name: Yup.string()
      .min(2, 'Имя должно состоять не менее чем из 2 символов')
      .matches(nameRegExp, 'Пожалуйста, введите правильное имя без пробелов в начале')
      .required('Имя обязательно'),
    email: Yup.string()
      .matches(emailRegExp, 'Пожалуйста, введите действительный адрес электронной почты')
      .required('Электронная почта обязательна'),
...
export const validationSchemaWaiverForm = isClientUnder18 => {
  return Yup.object().shape({
    ...
    governmentId: Yup.string()
      .matches(governmentId, 'Введите действительный идентификационный номер, выданный правительством')
      .required('Требуется удостоверение личности'),
    dob: Yup.string().required('Дата рождения обязательна'),
    address: Yup.string()
      .matches(
        addressRegex,
        'Пожалуйста, введите действительный адрес без пробелов.'
      )
      .required('Адрес обязателен'),
    bodyPart: Yup.string()
      .matches(nameRegExp, 'Пожалуйста, введите правильную часть тела')
